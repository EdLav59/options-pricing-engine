...
class MarketPriceAnalyzer:
    ...
    def analyze_pricing_errors(options_data, S, T, r):
        ...
        if len(significant_mispricing) > 0:
            print(f"\n⚠️  Found {len(significant_mispricing)} options with >5% pricing error")
...
def print_risk_management_report(params, sensitivity, hedging):
    ...
    if greeks['Gamma'] > 0.02:
        print("   ⚠️  High Gamma → Rehedge frequently (daily)")
    else:
        print("   ✓ Low Gamma → Rehedge less frequently (weekly)")
    
    if greeks['Vega'] > call_price * 0.05:
        print("   ⚠️  High Vega exposure → Consider vol hedging")
    
    if abs(greeks['Theta']) > call_price * 0.03:
        print("   ⚠️  Significant time decay → Monitor expiration risk")
